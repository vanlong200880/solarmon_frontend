...
export default class MainProjectService {
    ...
    static get instance() {
        if (!this._instance) {
            this._instance = new MainProjectService();
        }
     ...
     */
     getProjectSideBar(objE, callBack) {
        try {
            ...
            http.post(Constants.URL.MAIN_PROJECT.PROJECT_SIDEBAR, objE, function (status, rs) {
     ...
     */
     getListProjectByEmplpyee(objE, callBack) {
        try {
            var http = new CMSHttp(true);
            http.post(Constants.URL.MAIN_PROJECT.LIST_PROJECT_BY_EMPLOYEE, objE, function (status, rs) {
                if (typeof callBack === 'function') {
                    if (!status || !rs) {
                        //lỗi do http nên không làm gì vì đã có http thư viện xử lý
                        return;
     ...
     * @param  {Boolean} isShowProgress
     */
     getListMeterByEmplpyee(objE, callBack) {
        try {
            var http = new CMSHttp(true);
            http.post(Constants.URL.MAIN_PROJECT.LIST_METER_BY_EMPLOYEE, objE, function (status, rs) {
                if (typeof callBack === 'function') {
                    if (!status || !rs) {
                        //lỗi do http nên không làm gì vì đã có http thư viện xử lý
                        return;
     ...
     */
     getListPlantSummary(objE, callBack) {
        try {
            ...
            http.post(Constants.URL.MAIN_PROJECT.LIST_PLANT_SUMMARY, objE, function (status, rs) {
                if (typeof callBack === 'function') {
                    if (!status || !rs) {
                        //lỗi do http nên không làm gì vì đã có http thư viện xử lý
                        return;
     ...
     */
     getAllProjectByEmployeeId(objE, callBack) {
        try {
            var http = new CMSHttp(true);
            http.post(Constants.URL.MAIN_PROJECT.LIST_SCOPE, objE, function (status, rs) {
                if (typeof callBack === 'function') {
                    if (!status || !rs) {
                        //lỗi do http nên không làm gì vì đã có http thư viện xử lý
                        return;
