...
export default class ErrorTypeService {
    static _instance;
    static get instance() {
        if (!this._instance) {
            this._instance = new ErrorTypeService();
        }
    ...
    getList(objE, callBack) {
        try {
            ...
            http.post(Constants.URL.ERROR_TYPE.LIST, objE, function (status, rs) {
    ...
    updateStatus(objE, callBack) {
        try {
            ...
            http.post(Constants.URL.ERROR_TYPE.UPDATE_STATUS, objE, function (status, rs) {
                if (typeof callBack === 'function') {
                    if (!status || !rs) {
                        return;
                    }
    ...
    save(objE, callBack) {
        try {
            var http = new CMSHttp(true);
            http.post(Constants.URL.ERROR_TYPE.SAVE, objE, function (status, rs) {
                if (typeof callBack === 'function') {
                    if (!status || !rs) {
                        return;
                    }
    ...
    delete(objE, callBack) {
        try {
            var http = new CMSHttp(true);
            http.post(Constants.URL.ERROR_TYPE.DELETE, objE, function (status, rs) {
                if (typeof callBack === 'function') {
                    if (!status || !rs) {
                        return;
                    }
    ...
    getDetail(objE, callBack) {
        try {
            var http = new CMSHttp(true);
            http.post(Constants.URL.ERROR_TYPE.DETAIL, objE, function (status, rs) {
                if (typeof callBack === 'function') {
                    if (!status || !rs) {
    ...
    */
    getDropdownList(objE, callBack) {
        try {
            var http = new CMSHttp(false);
            http.post(Constants.URL.ERROR_TYPE.DROPDOWN_LIST, objE, function (status, rs) {
                if (typeof callBack === 'function') {
                    if (!status || !rs) {
                        return;
                    }
    ...
    */
     getDropdownListAttr(objE, callBack) {
        try {
            var http = new CMSHttp(false);
            http.post(Constants.URL.ERROR_TYPE.DROPDOWN_LIST_ATTR, objE, function (status, rs) {
                if (typeof callBack === 'function') {
                    if (!status || !rs) {
                        return;
                    }
