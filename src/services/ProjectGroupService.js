...
export default class ProjectGroupService {
    static _instance;
    static get instance() {
        if (!this._instance) {
            this._instance = new ProjectGroupService();
        }
    ...
    getList(objE, callBack) {
        try {
            ...
            http.post(Constants.URL.PROJECT_GROUP.LIST, objE, function (status, rs) {
    ...
    /**
      ...
      * @author long.pham
      * @param @param {ProjectGroupEntity} objE
    ...
    updateStatus(objE, callBack) {
        try {
            ...
            http.post(Constants.URL.PROJECT_GROUP.UPDATE_STATUS, objE, function (status, rs) {
                if (typeof callBack === 'function') {
                    if (!status || !rs) {
                        return;
                    }
    ...
    save(objE, callBack) {
        try {
            var http = new CMSHttp(true);
            http.post(Constants.URL.PROJECT_GROUP.SAVE, objE, function (status, rs) {
                if (typeof callBack === 'function') {
                    if (!status || !rs) {
                        return;
                    }
    ...
    delete(objE, callBack) {
        try {
            var http = new CMSHttp(true);
            http.post(Constants.URL.PROJECT_GROUP.DELETE, objE, function (status, rs) {
                if (typeof callBack === 'function') {
                    if (!status || !rs) {
                        return;
                    }
    ...
    getDetail(objE, callBack) {
        try {
            var http = new CMSHttp(true);
            http.post(Constants.URL.PROJECT_GROUP.DETAIL, objE, function (status, rs) {
                if (typeof callBack === 'function') {
                    if (!status || !rs) {
    ...
    */
    getDropdownList(objE, callBack) {
        try {
            var http = new CMSHttp(false);
            http.post(Constants.URL.PROJECT_GROUP.DROPDOWN_LIST, objE, function (status, rs) {
                if (typeof callBack === 'function') {
                    if (!status || !rs) {
                        return;
                    }
