...
export default class ClientActivitiesService {
    static _instance;
    static get instance() {
        if (!this._instance) {
            this._instance = new ClientActivitiesService();
        }
    ...
    getList(objE, callBack) {
        try {
            ...
            http.post(Constants.URL.CLIENT_ACTIVITIES.LIST, objE, function (status, rs) {
      ...
      */
      delete(objE, callBack) {
        try {
            var http = new CMSHttp(true);
            http.post(Constants.URL.CLIENT_ACTIVITIES.DELETE, objE, function (status, rs) {
                if (typeof callBack === 'function') {
                    if (!status || !rs) {
                        return;
                    }
      ...
      */
      closeAlert(objE, callBack) {
        try {
            var http = new CMSHttp(true);
            http.post(Constants.URL.CLIENT_ACTIVITIES.CLOSE_ALL, objE, function (status, rs) {
                if (typeof callBack === 'function') {
                    if (!status || !rs) {
                        return;
                    }
     ...
     */
      save(objE, callBack) {
        try {
            var http = new CMSHttp(true);
            http.post(Constants.URL.CLIENT_ACTIVITIES.SAVE, objE, function (status, rs) {
                if (typeof callBack === 'function') {
                    if (!status || !rs) {
                        return;
                    }
